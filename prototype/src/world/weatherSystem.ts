...
export class WeatherSystem {
  ...
  private forced: boolean = false; // 上帝模式强制天气标记
  ...
  advance(season: string, tick?: number, shichen?: string): void {
    ...
    if (this.forced) {
      ...
      if (tick !== undefined && shichen !== undefined) {
        this.state.historyDetail.push({ tick, weather: this.state.current, shichen });
        if (this.state.historyDetail.length > 30) this.state.historyDetail.shift();
      }
      return;
    }
  ...
  forceWeather(weather: WeatherType): void {
    ...
    this.forced = true; // 标记为强制天气，下次advance不覆盖
    // 更新连续计数
